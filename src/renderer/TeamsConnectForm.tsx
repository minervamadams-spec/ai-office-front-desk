...
import type { TeamsConnectInput } from '../shared/contracts';
import { Dismissible } from './CatalogHelpers';

export function TeamsConnectForm({ onConnected, dismissedNotices, onDismissNotice }: {
  onConnected: () => void; dismissedNotices: string[]; onDismissNotice: (id: string) => void;
}) {
  const [input, setInput] = useState<TeamsConnectInput>({ clientId: '', tenant: 'common' });
  ...
  async function connect() {
    ...
    const state = await window.frontDesk.teams.connect(input);
    setBusy(false);
    if (state.status === 'connected') onConnected(); else setStatus(state.lastError ?? 'Could not connect.');
  }

  return <div className="jira-form">
    <Dismissible id="teams-oauth-cost" dismissedNotices={dismissedNotices} onDismissNotice={onDismissNotice}>
      This app never charges you anything. Teams requests made with your sign-in count against your own Microsoft 365 account's own usage limits.
    </Dismissible>
    <p className="intro">Register a "Mobile and desktop applications" app in Microsoft Entra ID with a public-client redirect and the Chat.Read permission (an admin may need to approve it), then paste its client ID here — this can be the same app registration used for Outlook, with Chat.Read added. Sign-in happens in your system browser — this app never sees your Microsoft password.</p>
